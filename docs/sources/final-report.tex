...
Monitoring pipeline execution for malicious behavior is also difficult. Unlike traditional intrusion detection, there is no clean separation between expected and unexpected activity: the pipeline might be supposed to modify systems, spawn processes, and make network connections \cite{owaspPPE}. Even system call-level monitoring faces this same problem, since the injected payload runs inside the same process context as the build script and uses the exact same calls (spawning sub-processes, opening network connection and reading an environment variable) that any normal build step might want to do.

\subsection{Countermeasures}

Several countermeasures reduce the attack surface or makes the necessary malicious code much more complicated, however none completely eliminates the risk on their own. 

Requiring mandatory code review before changes reach the pipeline-tracked branch makes direct d-PPE from a single compromised account harder, as there must always be another \textit{human} involved in the process of merging changes to the "live config". However, this countermeasure does not necessarily prevent malicious code from being approved, as the human reviewing the code might not understand the malicious implication if it is well-engineered and obfuscated. With the rise of AI code reviewers, another attack vector is to target the reviewer itself through prompt injection embedded in the submitted code \cite{owaspLLM01}.

Scoping the credentials available to the pipeline to the minimum needed for its specific task also limits the damage if it is exploited \cite{owaspPPE5}. This might however greatly limit the practical use cases of the pipeline as it might not have access to credentials necessary to configure specific systems, networks or other parts of the infrastructure. 

Another countermeasure might be to set up limitations for the systems in the environment itself. That is for example limiting which other systems a specific host may access or what local resources they can read from. This countermeasure would actually make the demo of this project much harder as the exfiltration of information from the flag-holder container would not be possible over the network if it was limited in network connections. 

Making a mandatory step of testing changes in a so-called staging or development environment forces pipeline code to run in an isolated context first, where unexpected behavior such as unauthorized network connections or attempts to access production credentials could be detected \cite{owaspPPE}. However, a well-engineered payload can be made environment-aware: by checking for reachable endpoints or environment variables for example, it can remain dormant during staging and only activate once it reaches the production environment.

It is worth emphasizing that the relevance, feasibility and effectiveness of all countermeasures discussed above are \textbf{highly} dependent on the specific infrastructure and context in which the pipeline runs. A production system serving external customers through a DMZ presents a fundamentally different threat landscape than, for example, an air-gapped system. In the first case, outbound connections may go unnoticed among legitimate traffic; in the latter air-gapped system, the same behavior would be immediately halted or detected if the correct surveillance measures are in place. This means that there is no universal set of countermeasures for pipeline poisoning; the right mitigations must be derived from the security requirements, network topology and constraints of the specific environment.
...
\begin{itemize}
    ...
    \item An attack.sh script that runs the entire exploit from beginning to end and gets you the flag
